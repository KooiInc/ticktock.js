...
$.delegate(`click`, `#bttnOpenClose, details.chapter`, evt => {
  if (evt.target.closest(`#bttnOpenClose`)) {
    const theBttn = evt.target;
    const allOpen = theBttn.dataset?.allopen === '1' ?? false;
    $(`details.chapter`).each(el => {
      el.open = !allOpen;
    });
    return theBttn.dataset.allopen = allOpen ? `0` : `1`;
  }
  
  if (evt.target.closest(`.chapter`)) {
    setTimeout(() => {
...
const berlin = $D({timeZone: "Europe/Berlin"});
...
print(
  ...
  toDetailChapter(`locale/timeZone`,
    toDetailsBlock(
...
print(toDetailChapter(`Performance`,
  `<div><b class="warn">Note</b>: do <b><i class="warn">not</i></b> use TickTock.js (or use selectively)
      if you need to process tens of thousands of Dates &#128128;</div>`,
...
function toDetailChapter(summary, ...lemmas) {
  return `
    <details class="chapter">
      <summary><span><b>${summary}</b></span></summary>
      ${lemmas.join(``)}
    </details>`;
}
